...
Phase 1A 的 S1～S8 spike work items，以及 `A1-C1-01`～`A1-C1-04` Product Directory、Graph、entry boundary 与 joint Registry assembly 均已完成；当前没有 ready item。现有 assembly 不实现 Service/Event declarations、Registry source module、CLI 或 Runtime。S8 已在 Ubuntu 24.04 x64 通过 45 个行为检查、144 个 axe scan 与 20 个 Linux Chromium visual comparison。
