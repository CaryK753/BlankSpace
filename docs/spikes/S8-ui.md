...
`A1-S8-03` 的 Ubuntu capture run `34735655529` 使用 runner image `ubuntu24` / `20260907.300.1`、Node `24.20.0` 和 frozen pnpm lock。安装前 dry-run 精确核验 Chromium `1243`、Firefox `1543`、WebKit `2359`，其 executable SHA-256 分别为 `8c599d43…aa1e`、`062272ac…35b`、`a85baad3…c63b`。同一组 6 routes、7 navigation entries、4 commands 驱动两个结构不同的 Shell；扩展矩阵共 249 项，其中 209 通过，40 个仅限 Chromium 的视觉项在 Firefox/WebKit 按设计跳过。45 个 behavior checks 覆盖 route/deep-link/back、键盘 roving focus、skip link、44px 触控、local error、RTL、forced colors、reduced motion、长文本与 200% 等效 viewport；144 个 axe scans 为零 violation/零 incomplete；20 个 Linux Chromium baseline 全部生成。previous-minor fixture 兼容，Public Override 固定进入 `E_UI_OVERRIDE_REVIEW_REQUIRED` 人工门禁。精确固定 S1 依赖后，comparison run `34753664043` 在同一 runner image 上通过并将 lock-derived matrix hash 更新为 `bf2b5b50ac5585a8cc699f0b8091b218def24e2814ec463d9285fd61f41d892f`；UI fixture 与视觉基线未变化。

Artifact `10311127151` 成功 finalize，包含可分离评审文件；其 20 张 canonical PNG 与上一 runner image 的文件哈希逐一相同。代表性的两个 Shell、compact/expanded 与 light/dark 图像已人工检查。浏览器人工复核确认路由后焦点落在目标一级标题、live region 宣告 `Projects loaded`，实际放大后的 compact 布局没有横向裁切且关键动作仍可操作。最终 comparison run `34751706885` 在 commit `520ab2c243359f7d43d3f13bc8b59a2b1fa4214a` 上通过 209/40 矩阵、canonical evidence 零改写检查，并 finalize artifact `10316506341`。同一 commit 的 S2 `34751706900`、S3 `34751706970`、S4 `34751706886`、S5 `34751706897`、S6 `34751707014` 与 S7 `34751706903` 均通过。comparison workflow 不带 `--update-snapshots`，并用 `git diff --exit-code` 保证 CI 不改写 canonical evidence。
