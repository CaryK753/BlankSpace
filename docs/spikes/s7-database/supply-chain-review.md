...
`A1-S7-02` 的实际安装使用 `--save-exact --ignore-scripts`，最终 lock 解析 `@types/pg@8.23.1` 到仓库 Node 24 types line，没有引入 `@types/node@26`，也没有新增 `requiresBuild` 或安装脚本。exact package identity 已由 S7 transcript 在运行时再次读取并断言；Ubuntu 仍须通过 frozen-lock 安装复核同一解析。
